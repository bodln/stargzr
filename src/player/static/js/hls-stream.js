...
function loadHlsLibrary() {
  return new Promise((resolve, reject) => {
    ...
    s.src = HLS_JS_CDN;
    s.onload  = () => { debugLog("HLS.js loaded"); resolve(); };
...
function makeKeyLoader() {
  function KeyLoader(config) { this.config = config; }
  ...
  try {
    ...
    setTimeout(function() {
      try {
        ...
        debugLog("HLS: reattach failed (" + e.message + "), full restart in 1s");
        _recovering = false;
        if (_hlsRetryTimer) clearTimeout(_hlsRetryTimer);
        _hlsRetryTimer = setTimeout(function() { loadHlsStream(src); }, 1000);
      }
    }, 300);
  } catch(e) {
...
async function loadHlsStream(src) {
  ...
  try { await loadHlsLibrary(); }
  catch (e) { _showHlsBadge("HLS.js failed to load", "#dc3545"); debugLog("HLS: " + e.message); return; }
  ...
  if (Hls.isSupported()) {
    ...
    _hlsInstance.on(Hls.Events.MANIFEST_PARSED, function(ev, data) {
      ...
      video.play().catch(function(e) { debugLog("HLS autoplay blocked: " + e.message); });
      _startStallWatchdog(video, src);
    });

    _hlsInstance.on(Hls.Events.LEVEL_LOADED, function(ev, data) {
      debugLog("HLS: playlist refreshed, " + data.details.fragments.length + " frags");
    });

    _hlsInstance.on(Hls.Events.FRAG_LOADED, function(ev, data) {
      _lastProgress = Date.now();
      _showHlsBadge("LIVE", "#dc3545");
      debugLog("HLS: frag OK — " + (data.frag?.relurl || "").split("?")[0].split("/").pop());
    });
    ...
    _hlsInstance.on(Hls.Events.ERROR, function(ev, data) {
      const loc = data.frag?.relurl
        ...
        : data.url
          ...
          : "";
      debugLog("HLS [" + (data.fatal ? "FATAL" : "warn") + "] "
        + data.type + " / " + data.details + loc
      ...
      if (!data.fatal && data.details === Hls.ErrorDetails.BUFFER_STALLED_ERROR) {
        if (_recovering) {
          debugLog("HLS: buffer stalled during recovery — ignoring, waiting for reattach");
          return;
        }
        debugLog("HLS: buffer stalled — live-edge jump (startLoad -1)");
        _showHlsBadge("Recovering...", "#ffc107");
        _lastProgress = Date.now();
        try { _hlsInstance.stopLoad(); _hlsInstance.startLoad(-1); } catch(e) {
      ...
      if (!data.fatal && data.details === Hls.ErrorDetails.BUFFER_APPENDING_ERROR) {
        debugLog("HLS: MSE append error — SourceBuffer rejected segment, hard recovering");
        _hardRecoverHls(src);
        return;
      }
      ...
      if (data.type === Hls.ErrorTypes.NETWORK_ERROR) {
        ...
        if (_hlsRetryTimer) clearTimeout(_hlsRetryTimer);
        _hlsRetryTimer = setTimeout(function() { loadHlsStream(src); }, 3000);
      }
    });
    ...
    _hlsVideoEl = video;
    video.src = src;
    video.play().catch(function(e) { debugLog("HLS native: " + e.message); });
    _showHlsBadge("LIVE", "#dc3545");
  } else {
...
function _startStallWatchdog(video, src) {
  ...
  _stallWatchdog = setInterval(function() {
    ...
    if (secs > 12) {
      debugLog("HLS: stall " + secs.toFixed(1) + "s — startLoad(-1) failed, hard recovering");
      _hardRecoverHls(src);
    }
  }, 3000);
}
...
function closeHlsStream() {
  ...
  _stopStallWatchdog();
  _removeHlsBadge();
  if (_hlsRetryTimer) { clearTimeout(_hlsRetryTimer); _hlsRetryTimer = null; }
  _recovering = false;
  ...
  const closeBtn = document.getElementById("iframe-close-btn");
  if (closeBtn) { closeBtn.textContent = "Close"; closeBtn.classList.add("hidden"); closeBtn._hlsClose = false; }
...
function _destroyHls() {
  if (_hlsInstance) {
    try { _hlsInstance.stopLoad(); _hlsInstance.detachMedia(); _hlsInstance.destroy(); } catch(e) {}
    _hlsInstance = null;
  }
  if (_hlsVideoEl) {
    try { _hlsVideoEl.pause(); _hlsVideoEl.removeAttribute("src"); _hlsVideoEl.load(); } catch(e) {}
    _hlsVideoEl = null;
  }
}
...
function _showHlsBadge(text, color) {
  ...
  if (!badge) {
    ...
    badge.style.cssText = "position:fixed;top:10px;left:50%;transform:translateX(-50%);background:rgba(0,0,0,0.8);color:#fff;padding:5px 14px;border-radius:4px;font-size:13px;font-weight:bold;font-family:monospace;z-index:9999;pointer-events:none;border-left:4px solid " + color;
